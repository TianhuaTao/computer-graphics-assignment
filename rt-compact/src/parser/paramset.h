...
#ifndef RT_COMPACT_PARAMSET_H
#define RT_COMPACT_PARAMSET_H
...
public:
    ...
    Color FindOneSpectrum(const std::string &, const Color &d) const;
    ...
    const Color *FindSpectrum(const std::string &, int *nValues) const;
...
public:
    ...
    std::map<std::string, std::shared_ptr<Texture<Color>>> &sTex)
    : floatTextures(fTex),
    spectrumTextures(sTex),
    geomParams(geomParams),
    materialParams(materialParams) {}
    std::shared_ptr<Texture<Color>> GetSpectrumTexture(
            const std::string &name, const Color &def) const;
    std::shared_ptr<Texture<Color>> GetSpectrumTextureOrNull(
    ...
    Color FindSpectrum(const std::string &n, const Color &d) const {
...
private:
    ...
    std::map<std::string, std::shared_ptr<Texture<Color>>> &spectrumTextures;
    const ParamSet &geomParams, &materialParams;
};



#endif //RT_COMPACT_PARAMSET_H
