...
ParamSet parseParams(Next nextToken, Unget ungetToken, MemoryBank &arena,
...
static void parse(std::unique_ptr<Tokenizer> t) {
    ...
    // parameter and a ParamSet (e.g. apiShape()).
        ...
        switch (tok[0]) {
            case 'A':
                if (tok == "AttributeBegin")
                    ...
                    if (a == "All")
                        apiActiveTransformAll();
                    else if (a == "EndTime")
                        apiActiveTransformEndTime();
                    else if (a == "StartTime")
                        apiActiveTransformStartTime();
                    else
                        syntaxError(tok);
            ...
            case 'C':
                if (tok == "ConcatTransform") {
                    ...
                    if (nextToken(TokenRequired) != "]") syntaxError(tok);
                    apiConcatTransform(m);
                } else if (tok == "CoordinateSystem") {
                    string_view n = dequoteString(nextToken(TokenRequired));
                    apiCoordinateSystem(toString(n));
                } else if (tok == "CoordSysTransform") {
                    string_view n = dequoteString(nextToken(TokenRequired));
                    apiCoordSysTransform(toString(n));
                } else if (tok == "Camera")
                    basicParamListEntrypoint(SpectrumType::Reflectance, apiCamera);
                else
                    syntaxError(tok);
                break;
            ...
            case 'F':
                if (tok == "Film")
                    basicParamListEntrypoint(SpectrumType::Reflectance, apiFilm);
                else
                    syntaxError(tok);
                break;
            ...
            case 'L':
                if (tok == "LightSource")
                    ...
                    apiLookAt(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7],
                              v[8]);
                } else
                    syntaxError(tok);
                break;
            ...
            case 'M':
                if (tok == "MakeNamedMaterial")
                    ...
                    apiMediumInterface(names[0], names[1]);
                } else
                    syntaxError(tok);
                break;
            ...
            case 'O':
                if (tok == "ObjectBegin") {
                    string_view n = dequoteString(nextToken(TokenRequired));
                    apiObjectBegin(toString(n));
                } else if (tok == "ObjectEnd")
                    apiObjectEnd();
                else if (tok == "ObjectInstance") {
                    string_view n = dequoteString(nextToken(TokenRequired));
                    apiObjectInstance(toString(n));
                } else
                    syntaxError(tok);
                break;
            ...
            case 'R':
                if (tok == "ReverseOrientation")
                    apiReverseOrientation();
                else if (tok == "Rotate") {
                    Float v[4];
                    for (int i = 0; i < 4; ++i)
                        v[i] = parseNumber(nextToken(TokenRequired));
                    apiRotate(v[0], v[1], v[2], v[3]);
                } else
                    syntaxError(tok);
                break;
            ...
            case 'S':
                if (tok == "Shape")
                    basicParamListEntrypoint(SpectrumType::Reflectance, apiShape);
                else if (tok == "Sampler")
                    basicParamListEntrypoint(SpectrumType::Reflectance,
                                             apiSampler);
                else if (tok == "Scale") {
            ...
            case 'T':
                if (tok == "TransformBegin")
                    ...
                    for (int i = 0; i < 3; ++i)
                        v[i] = parseNumber(nextToken(TokenRequired));
                    apiTranslate(v[0], v[1], v[2]);
                } else if (tok == "TransformTimes") {
                    Float v[2];
                    for (int i = 0; i < 2; ++i)
                        v[i] = parseNumber(nextToken(TokenRequired));
                    apiTransformTimes(v[0], v[1]);
                } else if (tok == "Texture") {
                    string_view n = dequoteString(nextToken(TokenRequired));
                    ...
                    basicParamListEntrypoint(
                            ...
                            [&](const std::string &texName, const ParamSet &params) {
                                apiTexture(name, type, texName, params);
                            });
                } else
                    syntaxError(tok);
                break;
