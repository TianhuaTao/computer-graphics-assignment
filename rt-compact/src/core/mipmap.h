...
#ifndef RT_COMPACT_MIPMAP_H
#define RT_COMPACT_MIPMAP_H
...
/**
 * 实现 mip map 功能，用于 infinite light 的贴图功能
 * @tparam T
 */
template <typename T>
class MIPMap {
...
private:
    ...
    RGBColor clamp(const RGBColor &v) { return v.Clamp(0.f, Infinity); }
...
const T &MIPMap<T>::Texel(int level, int s, int t) const {
    ...
    const Array2D<T> &l = *pyramid[level];
    switch (wrapMode) {
