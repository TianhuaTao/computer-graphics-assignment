...
MetalMaterial::MetalMaterial(const std::shared_ptr<Texture<Color>> &eta,
...
void MetalMaterial::ComputeScatteringFunctions(SurfaceHit *si,
                                               MemoryBank &bank,
                                               TransportMode mode,
    ...
    Fresnel *frMf = BANK_ALLOC(bank, FresnelConductor)(1., eta->Evaluate(*si),
                                                       k->Evaluate(*si));
    MicrofacetDistribution *distrib =
            BANK_ALLOC(bank, TrowbridgeReitzDistribution)(uRough, vRough);
    si->bsdf->Add(BANK_ALLOC(bank, MicrofacetReflection)(1., distrib, frMf));
}
...
MetalMaterial *CreateMetalMaterial(const TextureParams &mp) {
    static Color copperN =
            Color::FromSampled(CopperWavelengths, CopperN, CopperSamples);
    std::shared_ptr<Texture<Color>> eta =
            mp.GetSpectrumTexture("eta", copperN);
    static Color copperK =
            Color::FromSampled(CopperWavelengths, CopperK, CopperSamples);
    std::shared_ptr<Texture<Color>> k = mp.GetSpectrumTexture("k", copperK);
